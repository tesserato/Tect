...
use crate::analyzer::{TectAnalyzer, TectParser, Rule};
...
impl LanguageServer for Backend {
                ...
                semantic_tokens_provider: Some(SemanticTokensServerCapabilities::SemanticTokensRegistrationOptions(
                    SemanticTokensRegistrationOptions {
                        text_document_registration_options: TextDocumentRegistrationOptions { 
                            document_selector: Some(vec![DocumentFilter { language: Some("tect".to_string()), scheme: Some("file".to_string()), pattern: None }]) 
                        },
                        semantic_tokens_options: SemanticTokensOptions {
                            work_done_progress_options: WorkDoneProgressOptions { work_done_progress: None },
                            legend: SemanticTokensLegend {
                                token_types: vec![
                                    SemanticTokenType::KEYWORD, SemanticTokenType::TYPE, SemanticTokenType::FUNCTION,
                                    SemanticTokenType::VARIABLE, SemanticTokenType::ENUM, SemanticTokenType::DECORATOR,
                                ],
                                token_modifiers: vec![],
                            },
                            range: Some(false),
    ...
    async fn did_open(&self, p: DidOpenTextDocumentParams) {
        self.document_map.insert(p.text_document.uri, p.text_document.text);
    }
    ...
    async fn hover(&self, p: HoverParams) -> LspResult<Option<Hover>> {
        ...
        let Some(content) = self.document_map.get(&uri) else { return Ok(None); };
            ...
            for cap in word_re.find_iter(line) {
                if pos.character >= cap.start() as u32 && pos.character <= cap.end() as u32 {
                    ...
                    let val = if let Some(info) = a.symbols.get(lookup) {
                        format!("### {}: `{}`\n**Type**: `{}`{}", info.kind, lookup, info.detail, info.docs.as_ref().map(|d| format!("\n\n---\n\n{}", d)).unwrap_or_default())
                    } else {
                        match lookup {
                    ...
                    return Ok(Some(Hover { 
                        contents: HoverContents::Markup(MarkupContent { kind: MarkupKind::Markdown, value: val }), 
                        range: Some(Range::new(Position::new(pos.line, cap.start() as u32), Position::new(pos.line, cap.end() as u32))) 
                    }));
                }
            }
        }
    ...
    async fn semantic_tokens_full(&self, p: SemanticTokensParams) -> LspResult<Option<SemanticTokensResult>> {
        ...
        if let Ok(pairs) = TectParser::parse(Rule::program, &content) {
            for pair in pairs.flatten() {
                let token_type = match pair.as_rule() {
                    Rule::kw_data | Rule::kw_error | Rule::kw_func | Rule::kw_for | Rule::kw_match | Rule::kw_in | Rule::kw_break | Rule::kw_group => Some(0),
                    Rule::type_ident => Some(match a.symbols.get(pair.as_str()).map(|s| s.kind.as_str()) { Some("Data") => 1, Some("Function") => 2, Some("Error") => 4, _ => 1 }),
                    Rule::var_ident => Some(match a.symbols.get(pair.as_str()).map(|s| s.kind.as_str()) { Some("Group") => 1, _ => 3 }),
                ...
                if let Some(idx) = token_type {
                    ...
                    tokens.push(SemanticToken { delta_line: delta_l, delta_start: delta_s, length: pair.as_str().len() as u32, token_type: idx, token_modifiers_bitset: 0 });
                    last_l = line; last_s = col;
                }
            }
        }
