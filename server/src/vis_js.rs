...
<head>
    ...
    <style type="text/css">
        ...
        /* Vis.js Dark Theme Controls Overrides */
...
<script type="text/javascript">
    ...
    const options = {{
        physics: {{ 
            ...
            forceAtlas2Based: {{ gravitationalConstant: -100, springLength: 10, avoidOverlap: 1, damping: 0.75 }} 
        }},
        interaction: {{ navigationButtons: true, keyboard: true, hover: true }},
        configure: {{
            enabled: true,
            container: configControls,
            showButton: false
        }}
    }};
    ...
    network.on("click", (p) => {{
        if (p.nodes.length > 0) {{
            ...
            if (network.isCluster(id)) network.openCluster(id);
                ...
                let d = nodes.get(id); 
                if (d && d.clusterGroup) network.cluster(clusterBy(d.clusterGroup)); 
            }}
        }}
    }});
