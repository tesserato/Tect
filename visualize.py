...
for kind, color in color_map.items():
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=color, 
                           node_size=3000, alpha=0.8, edgecolors='white', linewidths=2)
...
for edge in G.edges(data=True, keys=True):
    ...
    style = "solid"
    color = "#bdc3c7"
    if attr['label'] == "output_type": color = "#95a5a6"
    if attr['label'] == "argument_flow": style = "dashed"
    ...
    ax.annotate("",
                ...
                arrowprops=dict(arrowstyle="->", color=color,
                                ...
                                mutation_scale=20, lw=1.5, ls=style))

# 7. Edge Labels (Relations)
edge_labels = {(u, v): d['label'] for u, v, d in G.edges(data=True)}
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_size=8, label_pos=0.5, alpha=0.7)

# 8. Create Legend
legend_handles = [mpatches.Patch(color=color, label=kind) for kind, color in color_map.items()]
plt.legend(handles=legend_handles, loc='upper left', title="Node Types", frameon=True, fontsize=10)
